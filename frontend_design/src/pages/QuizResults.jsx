...
export default function QuizResults() {
  ...
  const stored = useMemo(() => {
    try { return JSON.parse(sessionStorage.getItem(`result-${id}`) || 'null'); } catch { return null; }
  }, [id]);
  const quiz = stored?.quiz || playQuiz;
    ...
    <main className="relative min-h-screen bg-[color:var(--bg)]" data-testid="results-main">
      <div className="sticky top-0 z-40 backdrop-blur-md bg-[color:var(--bg)]/70 border-b border-white/[0.05]">
        <div className="mx-auto max-w-[1100px] px-6 md:px-10 h-16 flex items-center justify-between">
          <Link to="/"><Wordmark size={22} /></Link>
          <Link to="/practice" className="text-[13.5px] text-[color:var(--ink-2)] hover:text-white transition-colors">Back to practice →</Link>
        </div>
      </div>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
            <div className="md:col-span-7">
              ...
              <FadeUp delay={0.3} className="mt-8 flex flex-wrap items-center gap-3">
                <Link to={`/play/${id}`} className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-[color:var(--violet)] hover:bg-[color:var(--violet-2)] text-white text-[13.5px] font-medium transition-colors" data-testid="results-retry"><RotateCw className="w-4 h-4" /> Try again</Link>
                <Link to="/practice" className="inline-flex items-center gap-2 px-5 py-3 rounded-full glass glass-hover text-[13.5px] text-white" data-testid="results-more">Explore more quizzes</Link>
