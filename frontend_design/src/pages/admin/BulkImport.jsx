...
export default function BulkImport() {
  ...
  return (
    <div data-testid="admin-import">
      ...
      <FadeUp delay={0.1}><h1 className="mt-4 font-display text-[48px] md:text-[64px] leading-[0.95] text-white">Bulk import</h1></FadeUp>
      <FadeUp delay={0.2} className="mt-2 text-[color:var(--ink-2)]">Upload a CSV or JSON of questions. Everything lands in the review queue first.</FadeUp>
      ...
      <FadeUp delay={0.25} className="mt-10">
        <label className="block rounded-2xl glass glass-hover border-dashed border-white/15 p-10 text-center cursor-pointer" data-testid="import-drop">
          ...
          <div className="mx-auto w-14 h-14 rounded-full grid place-items-center bg-[color:var(--violet)]/15 border border-[color:var(--violet)]/25"><UploadCloud className="w-6 h-6 text-[color:var(--violet-2)]" /></div>
          ...
          {file && (<div className="mt-5 inline-flex items-center gap-2 px-3 py-2 rounded-full glass text-[12.5px] text-white"><FileText className="w-4 h-4 text-[color:var(--mint)]" />{file.name}</div>)}
        </label>
      </FadeUp>
