...
export default function LiveRoom() {
  ...
  const [playerId] = useState(() => sessionStorage.getItem('hexquiz-player-id') || (() => {
    const id = Math.random().toString(36).slice(2, 10);
    sessionStorage.setItem('hexquiz-player-id', id);
    return id;
  })());
  const [room, setRoom] = useState(() => (codeParam ? loadRoom(codeParam) : null));
  ...
  // Reflect changes made from other tabs (other "players") joining/scoring.
  useEffect(() => {
    if (!codeParam) return;
    const onStorage = (e) => {
  ...
  const answer = useCallback((optionIdx) => {
    ...
    const players = room.players.map((p) => p.id === playerId
  ...
  const leaveRoom = () => { nav('/live'); };

  // --- Entry screen: no room code in the URL yet ---
  if (!codeParam) {
    return (
  ...
  // --- Room exists in the URL but not (yet, or no longer) in storage ---
  if (!room) {
    return (
  ...
  // --- Playing: synced round, each tab answers independently ---
  const q = ROUND[qIdx];
  const standings = [...room.players].sort((a, b) => b.score - a.score);
  return (
