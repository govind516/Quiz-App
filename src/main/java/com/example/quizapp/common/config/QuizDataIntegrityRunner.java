...
public class QuizDataIntegrityRunner implements ApplicationRunner {
	...
	private void backfillQuizTimeLimitSec() {
		...
		if (remainingNulls > 0) {
			log.warn("QuizDataIntegrityRunner: {} quizzes still have NULL question_time_limit_sec after backfill",
					remainingNulls);
		} else {
	...
	private void backfillQuizAttemptTimeLimitSec() {
		...
		if (remainingNulls > 0) {
			log.warn("QuizDataIntegrityRunner: {} quiz attempts still have NULL question_time_limit_sec after backfill",
					remainingNulls);
		} else {
