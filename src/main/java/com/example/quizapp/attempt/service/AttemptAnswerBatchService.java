...
public class AttemptAnswerBatchService {
	...
	public void batchInsert(List<AttemptAnswer> rows) {
		...
		try {
			// 1) Parent rows: single multi-value INSERT ... RETURNING id (Postgres)
			...
			// Use non-deprecated query signature: query(sql, RowMapper, args)
			...
			// H2 (test) does not support multi-value RETURNING — fallback to JPA batch
			// This keeps tests green while preserving Postgres-optimized path in prod
			attemptAnswerRepository.saveAll(rows);
		}
	}
}
