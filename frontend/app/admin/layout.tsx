...
function usePendingCount(): number | undefined {
  const q = useQuery({
  ...
  return q.data ? q.data.length : undefined;
}
...
const groups = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/admin/review", label: "Review queue", icon: Bell, key: "review", liveBadge: true },
    ],
  },
  {
    label: "Platform",
...
type SidebarItemType = (typeof groups)[number]["items"][number] & { badge?: number; liveBadge?: boolean; end?: boolean };

function SidebarItem({ item, isActive, badge }: { item: SidebarItemType; isActive: boolean; badge?: number }) {
  const Icon = item.icon;
  const shown = badge ?? (item as { badge?: number }).badge;
  return (
    <Link
      href={item.to}
                  ...
                  {groups.map((g) => (
                    <div key={g.label}>
                      ...
                      <div className="space-y-1">
                        {g.items.map((it) => {
                          const typed = it as SidebarItemType;
                          const isActive = typed.end ? pathname === it.to : pathname === it.to;
                          return <SidebarItem key={it.key} item={typed} isActive={isActive} badge={typed.liveBadge ? pendingCount : undefined} />;
                        })}
                      </div>
                    </div>
                  ))}
