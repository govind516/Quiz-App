...
import { usersList as mockUsersList } from "@/lib/mock";
import { api } from "@/lib/api";
import type { AdminUsersResponse } from "@/lib/types";

export default function Users() {
  // TODO: backend wiring — fetch real users; fallback to mock for pixel-perfect replica
        ...
        {usersList.map((u: any, i: number) => (
          ...
          >
            ...
            <div className="col-span-1">
              <span
                className={`px-2 py-0.5 rounded-md text-[11px] font-mono ${u.role === "Admin" ? "text-[color:var(--violet-2)] bg-[color:var(--violet)]/10 border border-[color:var(--violet)]/25" : u.role === "Editor" ? "text-[color:var(--gold)] bg-[color:var(--gold)]/10 border border-[color:var(--gold)]/25" : "text-[color:var(--ink-2)] bg-white/[0.04] border border-white/10"}`}
              >
                {u.role}
              </span>
            </div>
            ...
            <div className="col-span-1 text-right font-mono text-[12.5px] text-[color:var(--gold)]">
