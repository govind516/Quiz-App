  ...
  return (
    <div className="relative w-full h-[520px]">
      ...
      <motion.div drag dragElastic={0.08} dragConstraints={{ left: -20, right: 20, top: -20, bottom: 20 }}
        ...
        transition={{ delay: 0.8, duration: 0.9, ease: [0.16, 1, 0.3, 1] as any }}
...
function Stat({ n, l, delay=0 }: { n: any; l: any; delay?: number }) {
  return (
...
export default function Home() {
  ...
  const { scrollYProgress } = useScroll({ target: heroRef as any, offset: ['start start', 'end start'] });
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center">
            <div className="lg:col-span-6">
              ...
              <FadeUp delay={0.75} className="mt-10 flex flex-wrap items-center gap-3">
                <Link href="/practice" className="group inline-flex items-center gap-2 px-6 py-3.5 rounded-full bg-[color:var(--ink)] text-[#0A0A0F] text-[14px] font-medium hover:bg-white transition-all hover:shadow-[0_15px_40px_-10px_rgba(255,255,255,0.25)]" data-testid="hero-start-btn">
        ...
        <div className="mx-auto max-w-[1400px] px-6 md:px-10">
          <div className="flex items-end justify-between mb-14">
            ...
            <FadeUp delay={0.2} className="hidden md:block">
              <Link href="/practice" className="inline-flex items-center gap-1.5 text-[13.5px] text-[color:var(--ink-2)] hover:text-white transition-colors">Browse all <ChevronRight className="w-4 h-4" /></Link>
            </FadeUp>
          </div>
      ...
      <section className="relative py-20">
        <div className="mx-auto max-w-[1400px] px-6 md:px-10">
          <div className="flex items-end justify-between mb-12">
            ...
            <FadeUp delay={0.15}>
              <Link href="/practice" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full glass glass-hover text-[13px] text-white">Browse all <ChevronRight className="w-4 h-4" /></Link>
            </FadeUp>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {quizzes.slice(0, 3).map((q, i) => (
              <FadeUp key={q.id} delay={i * 0.08}>
                <Link href={`/play/${q.id}`} className="group relative rounded-2xl glass glass-hover p-6 h-full block" data-testid={`live-quiz-${q.id}`}>
        ...
        <div className="mx-auto max-w-[1400px] px-6 md:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            <div className="lg:col-span-4">
              ...
              <FadeUp delay={0.3} className="mt-8"><Link href="/leaderboard" className="inline-flex items-center gap-2 px-5 py-3 rounded-full glass glass-hover text-[13.5px] text-white">Full leaderboard <ArrowUpRight className="w-4 h-4" /></Link></FadeUp>
            </div>
        ...
        <div className="mx-auto max-w-[1400px] px-6 md:px-10">
          <div className="relative rounded-[32px] overflow-hidden glass p-10 md:p-16">
            ...
            <div className="relative flex flex-col md:flex-row items-start md:items-end justify-between gap-8">
              ...
              <Link href="/login" className="group inline-flex items-center gap-2 px-7 py-4 rounded-full bg-[color:var(--ink)] text-[#0A0A0F] text-[14px] font-medium hover:bg-white transition-all">Create your account <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" /></Link>
            </div>
          </div>
        </div>
      </section>
