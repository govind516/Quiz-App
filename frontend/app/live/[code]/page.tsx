...
export default function LiveCodeRoom() {
  ...
  const [room, setRoom] = useState<any>(() => (typeof window !== 'undefined' && codeParam ? loadRoom(codeParam) : null));
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === ROOM_PREFIX + codeParam) setRoom(loadRoom(codeParam));
    };
    window.addEventListener('storage', onStorage);
    const poll = setInterval(() => setRoom(loadRoom(codeParam)), 1000);
  ...
  const answer = useCallback((optionIdx: number) => {
    ...
    const players = room.players.map((p: any) => p.id === playerId ? { ...p, score: p.score + (correct ? 1 : 0), answered: p.answered + 1 } : p);
    const next = { ...room, players };
    saveRoom(next);
    setRoom(next);
    if (qIdx < ROUND.length - 1) setQIdx((i) => i + 1);
    else {
  ...
  const leaveRoom = () => router.push('/live');
    ...
    return (
      <main className="relative pt-36 pb-24" data-testid="live-not-found">
        <div className="mx-auto max-w-[600px] px-6 text-center">
          ...
          <p className="mt-2 text-[color:var(--ink-2)]">It may have been created in a different browser, or the code is wrong.</p>
      ...
      <main className="relative pt-36 pb-24" data-testid="live-lobby">
        ...
        <div className="relative mx-auto max-w-[720px] px-6 md:px-10">
          ...
          <FadeUp delay={0.3} className="mt-8 rounded-2xl glass overflow-hidden">
            ...
            {room.players.map((p: any) => (
              <div key={p.id} className="flex items-center gap-3 px-6 py-4 border-b border-white/[0.04] last:border-b-0">
                <div className="w-9 h-9 rounded-full grid place-items-center font-mono text-[11px] text-white/85 border border-white/10" style={{ background: 'linear-gradient(135deg, rgba(167,139,250,0.25), rgba(127,231,206,0.18))' }}>{p.name.slice(0,2).toUpperCase()}</div>
          ...
          <div className="mt-6 flex items-center gap-3">
            {isHost ? (
              <button onClick={startGame} data-testid="live-start-btn" className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-[color:var(--violet)] hover:bg-[color:var(--violet-2)] text-white text-[13.5px] font-medium transition-colors"><Play className="w-4 h-4" /> Start quiz for everyone</button>
            ) : (
    ...
    <main className="relative pt-36 pb-24" data-testid="live-playing">
      <div className="mx-auto max-w-[1000px] px-6 md:px-10 grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-8">
        <div>
          ...
          <div className="mt-3 rounded-3xl glass p-8"><div className="font-display text-[28px] leading-[1.2] text-white">{q.prompt}</div></div>
          <div className="mt-5 grid grid-cols-1 gap-3">
            {q.options.map((opt: string, i: number) => (
              <button key={i} onClick={()=>answer(i)} data-testid={`live-option-${i}`} className="text-left px-5 py-4 rounded-2xl border border-white/[0.06] hover:border-white/20 bg-white/[0.02] transition-all flex items-center gap-4">
